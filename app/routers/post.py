...
def get_posts(db: Session = Depends(get_db),
    ...
    results = (db.query(models.Post, func.count(models.Vote.post_id).label("total_vote"))
...
def get_post(id: int, db: Session = Depends(get_db),get_current_user: int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post, func.count(models.Vote.post_id).label("total_vote")).join(models.Vote, models.Vote.post_id == models.Post.id, isouter=True).group_by(models.Post.id).filter(models.Post.id == id).first()
...
def delete_post(id: int, db: Session = Depends(get_db), get_current_user: int = Depends(oauth2.get_current_user)):
    ...
    if post.owner_id != get_current_user.id:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"Not authorized to perform requestef action")
